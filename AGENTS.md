...
│
└── admin/                      # uni-admin 后台管理（H5 调试，待复测管理闭环）
```
...
> 版本 2.0.0 | 更新于 2026-07-03

### 整体状态：用户端核心链路已真机回归；admin 代码修复与首页二次重构完成，待 HBuilderX 复测管理闭环

| 根阶段 | 内容 | wallpaper/ | uniCloudWallPaper/ | crawler/ |
|--------|------|:---:|:---:|:---:|
| **阶段一** | 用户体系 | ✅ 阶段一 完成 | ✅ 阶段一 完成 | — |
| **阶段二** | 功能增强/架构重构 | ✅ 阶段二+三 完成 | ✅ 阶段二+三 完成 | ✅ 阶段一 完成 |
| **阶段三** | 后台管理 | — | ✅ 阶段四开发完成，待H5复测 | ✅ 阶段二 完成 |
| **阶段四** | 运营能力 | ✅ 阶段四 完成 | ⬜ 阶段五 2天 | ✅ 阶段三 完成 |
| **阶段五** | 测试发布 | ✅ 真机回归通过 | ⬜ 阶段六发布准备/管理闭环复测 | — |

### 各板块进度

| 板块 | 当前阶段 | 状态 | 下一步任务 |
|------|---------|------|-----------|
| **wallpaper/** | 阶段一~五 ✅ | 登录+互动+投稿+搜索+真机回归通过；可见品牌名已改为“一壁之力” | 发布前包体/隐私协议/体验版上传检查；改名后预览复核 |
| **uniCloudWallPaper/** | 阶段一~四 ✅ → 阶段五/六 | uni-id+互动API+云对象真机验证通过，admin已就绪；推送默认标题已改为“一壁之力” | 生产发布前确认公共模块、Schema、云对象最终部署；定时触发器/推送后续增强 |
| **crawler/** | ✅ 全部完成 | 去重+headless+健康检查+智能采集；本轮未改动 | 配合后端阶段五对接定时触发器 |
| **admin/** | ✅ 代码完成，待复测 | 修复启动空白页、云对象调用未匹配；9 个注册页面已补齐；首页按“一壁之力”运营工作台二次重构 | HBuilderX 内置浏览器复测管理闭环：列表加载、增删改审、刷新持久化、控制台无红错 |
...
| **admin/** | Vue 启动适配、云对象 URL 化调用桥接、品牌改名、运营工作台首页二次重构、壁纸管理（状态Tab+审核+分页+编辑）、分类/轮播图/公告/用户/评论管理、投稿审核（通过自动发布） |
...
| 后台 admin/ | ⬜ 待用户复测 | 代码侧已处理空白页、云对象未匹配和首页 UI；需运行到 HBuilderX 内置浏览器复测管理闭环 |
